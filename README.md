...
金讚鋼鐵與貼合加工廠房配置系統，支援廠區周邊示意（含 6 米環廠車道、西側花園與建物參考）、多樓層設計、獨立單面牆磁吸編輯、辦公室傢俱模組、SVG / DXF 匯出、即時歷史復原，以及 Supabase 跨裝置雲端同步。周邊示意參考衛星截圖，非地籍測量圖；使用前請以現場測量核對邊界與建物尺寸。

## V3.0.1 Email 與密碼登入

- 雲端帳號預設使用 Email＋密碼登入，另保留登入連結與忘記密碼／首次設定密碼。
- 已登入者可在帳號面板設定至少 12 字元的密碼；恢復連結會開啟密碼設定面板。
- 密碼交由 Supabase Auth 處理，不寫入配置檔或前端儲存空間。日常密碼登入不寄送 Email，首次設定／忘記密碼寄信仍受寄信服務限制。

## ◇ V3.0.0 2.5D Digital Twin

- 工具列「2.5D Twin」開啟日式 RPG 配色的等角廠房場景，直接使用目前本機或雲端檔案的配置。
- 支援 1F／2F、設備旋轉、柱子、牆面、夾層區域、走道與動線；拖曳巡覽、滑鼠滾輪／雙指縮放及四向視角。
- 點選物件或使用下拉清單查看尺寸與座標，按「在平面圖編輯」回到原有編輯器。平面圖變更與接收的雲端配置會更新場景。
- 介面支援繁中／泰文。平面尺寸使用原始資料；物件高度為示意值，尚未連接 IoT 或即時機台訊號。

## ↻ V2.9.6 設備旋轉修正

- 修正原始配置、舊雲端資料或匯入設備缺少 `rotation` 欄位時，側邊欄 ±90° 按鈕沒有反應的問題。
- 缺少角度的設備與柱子會從 0° 開始旋轉，文字格式的舊角度也會先轉為數字，並正常儲存及同步。
